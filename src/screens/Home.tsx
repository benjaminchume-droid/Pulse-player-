...
import { Play, Sparkles, Flame, Headphones, Star, Zap, Library, Calendar } from 'lucide-react';
...
export const Home: React.FC = () => {
  ...
  const sortedByRecent = [...songs].sort((a, b) => b.addedAt - a.addedAt);
  ...
  return (
    <div className="min-h-screen pb-44 pt-10 px-6 space-y-9 overflow-y-auto no-scrollbar relative">
      ...
      >
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-light tracking-tight text-white font-display">
            {getGreeting()}, <span className="font-semibold text-glow">Developer</span>
          </h1>
        </div>
      ...
      >
        ...
        <div className="relative z-10 space-y-4">
          ...
          <div className="space-y-1.5">
            <h2 className="text-3xl font-light text-white leading-tight font-display tracking-tight">
              Aura Ambient Shaders
            </h2>
            <p className="text-xs text-white/40 uppercase tracking-[0.2em]">Mix of the century • 12 Tracks</p>
          </div>

          <button className="px-6 py-2.5 rounded-full bg-white text-black text-[9px] font-bold uppercase tracking-[0.25em] flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-glow">
      ...
      <section className="space-y-4">
        ...
        <div className="p-4 rounded-2xl glass-card space-y-3">
          <div className="flex justify-between items-center">
            <p className="text-xs text-white/80 font-medium">Quest: Listen to 3 tracks today</p>
            <span className="text-[10px] font-mono font-bold text-white">2/3 Done</span>
          </div>
          <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
            <div className="w-2/3 h-full bg-white shadow-[0_0_10px_rgba(255,255,255,0.7)]" />
          </div>
        </div>
      </section>
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 -mx-6 px-6">
          {sortedByRecent.map((song, idx) => (
            <motion.div
              ...
              onClick={() => play(song)}
              className="w-36 flex-shrink-0 group cursor-pointer space-y-3"
            >
      ...
      <section className="space-y-5">
        <div className="flex justify-between items-center">
          <h3 className="text-sm font-bold uppercase tracking-[0.25em] text-white flex items-center gap-2">
            <Star size={14} className="text-white/40 animate-pulse" />
            ...
            {favorites.slice(0, 4).map((song) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[9px] text-white/30 truncate uppercase tracking-widest mt-0.5">{song.artist}</p>
                </div>
              </div>
            ))}
          </div>
        ) : (
          <div className="py-8 text-center rounded-2xl border border-dashed border-white/10 text-white/20 text-xs font-bold uppercase tracking-[0.2em]">
